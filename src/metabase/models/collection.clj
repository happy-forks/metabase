...
(defn- pre-update [{collection-name :name, id :id, color :color, archived? :archived, :as collection}]
  ...
  (db/update-where! 'Card {:collection_id id}
    :archived archived?)
  ;; For Personal Collections, check that any changes we're trying to make are ok
  (when (contains? collection :personal_owner_id)
    (check-changes-allowed-for-personal-collection collection))
...
(s/defn ^:private user->personal-collection-name :- su/NonBlankString
  ...
  (let [{first-name :first_name, last-name :last_name} (db/select-one ['User :first_name :last_name]
    ...
    (tru "Collection for {0} {1}" first-name last-name)))
